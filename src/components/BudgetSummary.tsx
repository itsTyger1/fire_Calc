...
export function BudgetSummary({ data, selectedScenario, selectedBudget, comparisonBudgets, setSelected }: {
  ...
  return (      <div className="budget-summary">
        <Section title="Monthly budget breakdown" eyebrow={`${base.scenario.name} · ${selectedBudget.phase?.name ?? 'Base contributions'}`}>
          <div className="scale-explanation"><strong>What counts here</strong><span>Deposited take-home is the pay received in your bank account. Every expense below, Roth IRA contributions, taxable brokerage contributions, and HYSA/cash savings are subtracted from it. Roth 401(k) contributions are shown separately because payroll already withheld them. Edit these amounts in Monthly money at the top.</span></div>
          <div className="budget-bars">{[
            ['Needs', selectedBudget.needs, data.profile.budgetTargets.needs, '#65a7ff'], ['Wants', selectedBudget.wants, data.profile.budgetTargets.wants, '#f4b860'], ['Take-home savings & investments', selectedBudget.takeHomeContributions, data.profile.budgetTargets.wealth, '#37d39a'],
          ...
          <p className="fine-print">Formula: deposited take-home − expenses − Roth IRA − taxable brokerage − HYSA/cash savings = unassigned take-home. Payroll and employer contributions are not deducted again. Other account types remain outside this budget.</p>
